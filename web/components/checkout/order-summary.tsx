...
export default function OrderSummary({
  ...
  const handleCouponChange = (value: string) => {
    ...
    if (
      appliedCoupon &&
      value.trim().toLowerCase() !== appliedCoupon.code.toLowerCase()
    ) {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      setAppliedCoupon(null);
      setCouponError(
        error instanceof Error ? error.message : "No pudimos validar el cupón",
      );
    } finally {
  ...
  return (
    <div className="border border-bone bg-parchment p-6">
      <h2 className="mb-6 text-xl font-medium font-serif">Resumen de Compra</h2>

      <ul className="mb-6 space-y-4">
        {cart.lines.length === 0 && (
          <li className="bg-bone/40 px-4 py-6 text-center text-sm text-stone-brown">
        ...
        {cart.lines.map((item) => {
          ...
          const colorImage = colorOption
            ? item.merchandise.product.colorImages?.find(
                (ci) =>
                  ci.color.toLowerCase() === colorOption.value.toLowerCase(),
              )?.url
            : null;

          const imageUrl =
            colorImage || item.merchandise.product.featuredImage.url;
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-x-2 gap-y-1">
                    {item.merchandise.selectedOptions.map((option, index) => (
                      <p
                        key={index}
                        className="text-[10px] text-stone-brown uppercase tracking-wider"
                      >
                        <span className="font-semibold text-stone-brown/60">
                          {option.name}:
                        </span>{" "}
                        {option.value}
                      </p>
                ...
                <div className="mt-3 flex h-8 w-fit items-center border border-bone">
      ...
      <div className="space-y-2 border-t border-bone pt-6 text-sm">
          ...
          <div className="mb-4 space-y-2 text-xs">
            {shopConfig.min_quantity > 0 && (
              ...
              >
                ...
                <span>
                  Mínimo <strong>{shopConfig.min_quantity}</strong> prenda
                  {shopConfig.min_quantity !== 1 ? "s" : ""}
                </span>
              </div>
            )}
            {shopConfig.min_amount > 0 && (
              ...
              >
                ...
                <span>
                  ...
                  <strong>
                    $
                    {Number(shopConfig.min_amount).toLocaleString("es-AR", {
        ...
        <div className="space-y-2 py-2">
          ...
          <div className="flex h-10 overflow-hidden border border-bone bg-bone/20">
            <input
              value={couponCode}
