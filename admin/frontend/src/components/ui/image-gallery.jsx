...
const uniqueImageFileName = (file) => {
...
function SortableImage({ image, onRemove, isCover }) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      ...
      />
      {isCover && (
        <span className="absolute left-2 top-2 z-10 rounded-full bg-primary px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-primary-foreground shadow-sm">
          Portada
        </span>
      )}
      <div
...
export function ImageGallery({
  ...
  const handleCropSave = async () => {
    ...
    try {
      ...
      const file = new File(
        [croppedImageBlob],
        uniqueImageFileName(activeFile),
        {
          type: "image/jpeg",
        },
      );
