...
function getProductsListReturnUrl() {
  ...
  try {
    ...
    const basename = "/vadmin";
    if (stored.startsWith(basename)) {
      return stored.slice(basename.length) || "/";
    }
    return stored;
  } catch {
    return "/productos";
...
export default function ProductForm() {
  ...
  const generateVariants = () => {
    ...
    if (selectedSizeIds.length === 0 || selectedColorIds.length === 0) {
      toast.error(
        "Debes seleccionar al menos un talle y un color para generar variantes",
      );
      return;
    }
    ...
    colorIds.forEach((colorId) => {
      sizeIds.forEach((sizeId) => {
        ...
        const exists = currentVariants.find(
          (v) => v.product_color_id == colorId && v.product_size_id == sizeId,
        );
        ...
        if (exists) {
          ...
          if (colorObj)
            generatedSku += `-${colorObj.name.toUpperCase().replace(/\s+/g, "")}`;
          if (sizeObj)
            generatedSku += `-${sizeObj.name.toUpperCase().replace(/\s+/g, "")}`;
  ...
  const applyToAllMinStock = (value) => {
    const variants = form
      .getValues("variants")
      .map((v) => ({ ...v, min_stock: parseInt(value) || 0 }));
  ...
  const applyToAllStock = (value) => {
    const variants = form
      .getValues("variants")
      .map((v) => ({ ...v, stock: parseInt(value) || 0 }));
    form.setValue("variants", variants);
    toast.info("Stock aplicado a todas las variantes");
  };
  ...
  const onSubmit = (values) => {
    ...
    gallery.forEach((img, index) => {
      if (img.file) {
        ...
        galleryOrder[String(img.id)] = index;
      }
    });
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      ...
                      <FormControl>
                        <Input
                          {...field}
                          placeholder={"URL amigable (autogenerado)"}
                          value={field.value || ""}
                        />
                      </FormControl>
                      <FormMessage />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-lg border border-border bg-muted/20 p-4 space-y-3">
                  ...
                  <div className="rounded-md border bg-background/50 px-3 py-2 text-sm">
                    ...
                    <span className="font-semibold">
                      {formatPrice(
                        calculateFinalPrice(salePrice, retailDiscount),
                      )}
                    </span>
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-border bg-muted/20 p-4 space-y-3">
                  ...
                  <div className="rounded-md border bg-background/50 px-3 py-2 text-sm">
                    ...
                    <span className="font-semibold">
                      {formatPrice(
                        calculateFinalPrice(wholesalePrice, wholesaleDiscount),
                      )}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="flex flex-wrap items-end gap-4">
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex overflow-hidden h-9 border rounded-md bg-muted/20">
                        ...
                        >
                          <Check
                            className={`h-4 w-4 ${bulkValues.stock ? "text-green-600 font-bold" : "text-muted-foreground"}`}
                          />
                        </Button>
                      </div>
                    </div>
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex overflow-hidden h-9 border rounded-md bg-muted/20">
                        ...
                        <Button
                          ...
                          className="rounded-none border-l h-full px-3 hover:bg-muted"
                          onClick={() =>
                            applyToAllMinStock(bulkValues.minStock)
                          }
                        >
                          <Check
                            className={`h-4 w-4 ${bulkValues.minStock ? "text-green-600 font-bold" : "text-muted-foreground"}`}
                          />
                        </Button>
                      </div>
                    </div>
                    ...
                    <TableBody>
                      {variants.map((variant, index) => {
                        const color = colors.find(
                          (c) => c.id == variant.product_color_id,
                        );
                        const size = sizes.find(
                          (s) => s.id == variant.product_size_id,
                        );
                        return (
                          <TableRow key={index}>
