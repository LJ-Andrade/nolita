...
export default function ResetPassword() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await axiosPublic.post("/password/reset", formData);
			const message = response.data.message;

			const translations = {
				"passwords.reset": "¡Tu contraseña ha sido restablecida!",
				"passwords.token": "Este token de restablecimiento de contraseña es inválido.",
			...
			toast.success(translations[message] || message || "Tu contraseña ha sido restablecida exitosamente.");
			...
			toast.error(translations[message] || message || "Ocurrió un error al restablecer tu contraseña.");
		} finally {
			setLoading(false);
		}
	};
		...
		<div className="min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden bg-black">
			...
			<div className="relative z-10 w-full max-w-md">
				...
				<Card className="bg-slate-900/92 backdrop-blur-xl border-none shadow-2xl" style={{ boxShadow: '0 8px 40px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.08)' }}>
					<CardHeader>
						<CardTitle className="text-2xl font-semibold text-center">Nueva Contraseña</CardTitle>
					...
					<form onSubmit={handleSubmit}>
						<CardContent className="space-y-4 pt-4">
							<div className="space-y-2">
								<Label htmlFor="password" className="text-sm font-medium text-slate-300">Nueva Contraseña</Label>
								<div className="relative">
									...
									<Input
										...
										required
										disabled={!formData.token || loading}
									/>
								</div>
							</div>

							<div className="space-y-2">
								<Label htmlFor="password_confirmation" className="text-sm font-medium text-slate-300">Confirmar Contraseña</Label>
								<div className="relative">
